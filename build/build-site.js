...
/*!
 * Site Builder Script for Chassis Assets
 *
 * Comprehensive build tool for managing Chassis Assets documentation site.
 * Handles asset building, Astro site building, and deployment validation.
 *
 * Usage:
 ...
 *   assets    Build assets for distribution
...
/**
 * Chassis Assets site builder with asset distribution management
 */
class ChassisBuilder {
  ...
  checkDependencies() {
    ...
    const requiredDirs = [
      ...
      { path: path.join(this.rootDir, 'source'), name: 'source (asset source files)' }
    ]
   ...
   */
  buildAssets() {
    this.log('Building assets for distribution...', 'info')

    try {
      // Build the assets using the project's build script
      this.runCommand('pnpm assets:site')

      // Verify the build succeeded
      this.log('Verifying asset build output...', 'info')
      const expectedPath = path.join(this.distDir, 'web/chassis-docs')

      if (fs.existsSync(expectedPath)) {
        const contents = fs.readdirSync(expectedPath)
        this.log(`✓ Found chassis-docs assets: ${contents.join(', ')}`, 'success')
      } else {
        this.log('⚠️  dist/web/chassis-docs not found, checking dist structure...', 'warning')
        if (fs.existsSync(this.distDir)) {
          const platforms = fs.readdirSync(this.distDir)
          this.log(`   Available platforms: ${platforms.join(', ')}`, 'info')
        }
      }
...
function main() {
  ...
  try {
    switch (command) {
      ...
      case '-h': {
        console.log(`
ChassisBuilder - Chassis Assets Site Builder
